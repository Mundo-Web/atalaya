...
const StartYeti = () => {
  ...
  var mySVG = document.querySelector('.logo-img');
  ...
  function morphTo(pathElement, targetPath, duration) {
    gsap.to(pathElement, {
      duration: duration,
      attr: { d: targetPath },
      ease: "expo.out"
  ...
  function onEmailInput(e) {
    ...
    if (curEmailIndex > 0) {
      if (mouthStatus == "small") {
        mouthStatus = "medium";
        morphTo(mouthBG, mouthMediumBG.getAttribute('d'), 1);
        morphTo(mouthOutline, mouthMediumBG.getAttribute('d'), 1);
        morphTo(mouthMaskPath, mouthMediumBG.getAttribute('d'), 1);
      ...
      if (value.includes("@")) {
        mouthStatus = "large";
        morphTo(mouthBG, mouthLargeBG.getAttribute('d'), 1);
        morphTo(mouthOutline, mouthLargeBG.getAttribute('d'), 1);
        morphTo(mouthMaskPath, mouthLargeBG.getAttribute('d'), 1);
      ...
      mouthStatus = "small";
      morphTo(mouthBG, mouthSmallBG.getAttribute('d'), 1);
      morphTo(mouthOutline, mouthSmallBG.getAttribute('d'), 1);
      morphTo(mouthMaskPath, mouthSmallBG.getAttribute('d'), 1);
      gsap.to(tooth, 1, { x: 0, y: 0, ease: "expo.out" });
  ...
  function coverEyes() {
    gsap.to(armL, .45, { x: -93, y: 2, rotation: 0, ease: "quad.out" });
    gsap.to(armR, .45, { x: -93, y: 2, rotation: 0, ease: "quad.out", delay: .1 });
  }
  ...
  function uncoverEyes() {
    ...
    gsap.to(armR, 1.35, { rotation: -105, ease: "quad.out", delay: .1 });
  }
